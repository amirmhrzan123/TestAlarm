...
@BindingMethods(@BindingMethod(type = EasySwipeMenuLayout.class, attribute = "app:onSwipe", method = "setOnSwipeListener"))
public class EasySwipeMenuLayout extends ViewGroup {
    ...
    public void setSwipeMenuListener(final OnSwipeMenuListener mSwipeMenuListener) {
...
//                }
                ...
                //越界修正
                if (getScrollX() < 0) {
                    if (!mCanLeftSwipe || mLeftView == null) {
                        scrollTo(0, 0);
                    } else {//左滑
                    ...
                    if (!mCanRightSwipe || mRightView == null) {
                        scrollTo(0, 0);
                    } else {
...
//                }
    ...
    private void handlerSwipeMenu(State result) {
        if (result == State.LEFTOPEN) {
            ...
            mViewCache = this;
            mStateCache = result;
            Log.i(TAG, "handlerSwipeMenu: + export seen");
            if (mSwipeMenuListener != null)
                mSwipeMenuListener.onSwipe();
